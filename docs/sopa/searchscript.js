  ...
  */
  var WordFind = function () {
    ...
    */
    ...
    // var allOrientations = datas;
    var allOrientations = ['horizontalBack','verticalUp','horizontal','vertical',
                           'diagonal','diagonalUp','diagonalBack','diagonalUpBack'];
   /* if(allOrientations == "horizontal")
    {
      allOrientations = "diagonal";
    }
    if(allOrientations == "vertical")
    {
      datas = "diagonal";
    }
*/
    ...
    var findBestLocations = function (puzzle, options, word) {
      ...
      for (var k = 0, len = options.orientations.length; k < len; k++) {
        ...
        var orientation = options.orientations[k],
            ...
            x = 0, y = 0;
         /* if(orientation[k] == "horizontal")
          {
            orientation[k] = "vertical";
          }
          if(orientation[k] == "vertical")
          {
    ...
    return {
      ...
      */
      newPuzzle: function(words, settings) {
       ...
       // console.log(words);
        ...
        var options = {
          height:       opts.height || heights,
          width:        opts.width || lenghts,
      ...
      */
      /*solve: function (puzzle, words) {
       ...
       solve: function (puzzle, words, options) {
                        ...
                        for (var i = 0, len = words.length; i < len; i++) {
                            var word = words[i],
                                locations = findBestLocations(puzzle, options, word);
                            // console.log("  " + i + " "+ word + " analisis");
                            for (var li = 0; li < locations.length; li++) {
                                if (locations[li].overlap === word.length) {
                                    locations[li].word = word;
                                    found.push(locations[li]);

                                   // console.log("\t" + i + " " + word + " found at [" + locations[li].x + "x" + locations[li].y + "] " + locations[li].orientation);
                                }
                                else {
                                    notFound.push(word);
                                   // console.log("\t" + i + " "+ word + " not found");
                                }
                            }
                        }
  ...
  var WordFindGame = function() {
    ...
    */
    var drawWords = function (el, words) {
      // var output = '<ul class="row">';
      var output = '<div class="row">';
      for (var i = 0, len = words.length; i < len; i++) {
        var word = words[i];
        var order='';
        order= i%2 == 0 ? "even" : "odd";
        // output += '<li class="word ' + word + ' col-md-5 '+ order +' ">' + word;
        if (i>0)
          output += "  -  ";
        output += word;
      }
    ...
    return {
      ...
      */
      ...
      solve: function (puzzle, words, options) {
                        ...
                        var solution = wordfind.solve(puzzle, words, options).found;
